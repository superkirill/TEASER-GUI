...
import os
import datetime
from smooth_net.core import ops
...
class NetworkBuilder(object):
    ...
    def _build_data_loader(self):
        print(self.config.training_data_folder)
    ...
    def _build_model(self):
        ...
        from smooth_net.core.architecture import network_architecture
    ...
    def _build_loss(self):
        ...
        from smooth_net.core import loss
    ...
    def test(self):
        ...
        evaluation_files = [self.config.parametrization1, self.config.parametrization2]
        ...
        for file in evaluation_files:
            ...
            np.savez_compressed(self.config.evaluate_output_folder + '{}_dim/'.format(self.config.output_dim) +
                                evaluate_file_name.split('/')[1][:-4] + '.npz', data=all_predictions)

            np.savetxt(self.config.evaluate_output_folder + '{}_dim/'.format(self.config.output_dim) +
                       evaluate_file_name.split('/')[1][:-4] + '.txt', all_predictions, delimiter=',', encoding=None)

            print('Wrote file {0}'.format(evaluate_file_name.split('/')[1][:-4] + '.npz'))
